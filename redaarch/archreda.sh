...
echo -e "extracting the UUID of the root partition using a bit of awk magic\n"
sleep 3
# downloading bat in the live env to easily get the line number for the UUID
echo -e "downloading bat in the live env to easily get the line number for the UUID\n"
sleep 3
...
do
    ...
    clear
    bat /mnt/etc/fstab
    echo -e "the root UUID is  : $root_uuid \n"
    read -p "Is that accurate? [yes/No] " answer
    if [ $answer = "yes" ]
    then
        echo -e "you chose yes\n"
        uuid_target_acquired=1
        sleep 3
        clear
    else
        echo -e "let's go again\n"
        sleep 3
        clear
    fi
done
...
EOF
clear
echo -e "here is the the boot entery:\n"
sleep 2
